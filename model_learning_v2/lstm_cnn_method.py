...
def main():
    ...
    if GENERATE == 'neighbor':
        requested_order_data = np.load(os.path.join(PARENT_DIR, 'Data_sys_20_grid/generated_orders_neighbors_%d_%d.npy' % (BEGIN_HOUR, END_HOUR)))
        true_arrival_rate = np.load(os.path.join(PARENT_DIR, 'Data_sys_20_grid/lam_true_neighbors_%d_%d.npy' % (BEGIN_HOUR, END_HOUR)))
    else:
        requested_order_data = np.load(os.path.join(PARENT_DIR, 'Data_sys_20_grid/generated_orders_%d_%d.npy' % (BEGIN_HOUR, END_HOUR)))
        true_arrival_rate = np.load(os.path.join(PARENT_DIR, 'Data_sys_20_grid/lam_true_%d_%d.npy' % (BEGIN_HOUR, END_HOUR)))

    neighbor_grids_table = np.load(os.path.join(PARENT_DIR, 'Data_sys_20_grid/neighbors_table.npy')).astype(int)
