...
GENERATED_TRANSITION_MATRIX = os.path.join(
    PARENT_DIR, 'Data_sys/generated_transition_matrix_')
GENERATED_TRANSITION_MATRIX_V2 = os.path.join(
    PARENT_DIR, 'Data_sys/generated_transition_matrix_v2.npy')
...
def main():
    ...
    #     generated_data[i, :, :, :] = np.squeeze(np.load(GENERATED_TRANSITION_MATRIX + '%s' % i + '.npy'))
    generated_data = np.load(GENERATED_TRANSITION_MATRIX_V2)

    mean_data = np.mean(generated_data, axis=0)  # size (NUM_SLOTS_FOR_ONE_DAY, NUM_GRIDS, NUM_GRIDS)
    np.save('pred_baseline_transition.npy', mean_data)
    ...
    q_network = CNNTransition().to(device)
    ...
    torch.save(q_network.state_dict(), 'pred_transition_CNN.pkl')  # save parameters
